...
def word_preprocess(doc_sent, intersection_set):
    idx_sent = {}
    idx_word = {}
    voc_set = set()
    for idx, sents in enumerate(doc_sent.items()):
        ...
        for sent in sents[1]:
            ...
            for w in word:
                if w in intersection_set:
                    intersection_word.append(w)
            sent_word[sent] = intersection_word
            words.append(intersection_word)
...
def main():
    ...
    with open('covid_voc.pickle', 'rb')as fpick:
        covid_voc = pickle.load(fpick)
    with open('virus_voc.pickle', 'rb')as fpick:
        virus_voc = pickle.load(fpick)
    intersection_set = covid_voc.intersection(virus_voc)
    print(len(intersection_set))

    doc = read_xmlfile(path)
    article_idx = idx_sent(doc)
    doc_idx = split_sent(article_idx)
    sents_word, idx_word, voc_set= word_preprocess(doc_idx, intersection_set) # {文章 : {sentence : split sentence}} | {文章 : words}
    print(sents_word)
